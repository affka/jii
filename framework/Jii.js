...
// Mix underscore string to lodash
...
var self = Joints.defineClass('Jii', Joints.Object, {}, {
    ...
    init: function(config) {
        ...
        this.aliases = {
            // @todo Set for client
            '@jii': typeof __dirname !== 'undefined' ? __dirname : location.host
        };
    ...
    configure: function(object, config) {
        for (var key in config) {
            ...
            if (_.isFunction(object[key])) {
                throw new Jii.exceptions.InvalidConfigException('You can not replace from config function `' + key + '` in object `' + object.debugClassName + '`.');
            }
            ...
            if (_.isUndefined(object[key]) && !_.isFunction(object[setter])) {
                throw new Jii.exceptions.InvalidConfigException('Config param `' + key + '` is undefined in object `' + object.debugClassName + '`.');
            }
            if (!_.isUndefined(object[key]) && _.isFunction(object[setter])) {
                throw new Jii.exceptions.InvalidConfigException('You have two setters (function and public param) for config param `' + key + '` in object `' + object.debugClassName + '`.  Please change param access (to `_' + key + '`) or remove setter method.');
            }
