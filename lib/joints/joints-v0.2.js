(function () {
     ...
     * @param {Function|Function[]} parentClass
    ...
    Joints.defineClass = function (globalName, parentClass, prototypeProperties, staticProperties) {
        var mixins = [];
        if (_.isArray(parentClass)) {
            mixins = parentClass;
            parentClass = parentClass[0];
            mixins = _.rest(mixins);
        }
        ...
        // Apply prototype mixins
        _.each(mixins, function(mixin) {
            if (_.isFunction(mixin)) {
                mixin = mixin.prototype;
            }

            _.extend(newClass.prototype, mixin);
        });
     ...
     * @return {function}
     */
    Joints.defineMixin = function (globalName, staticProperties) {
        // Split namespace
        var pos = globalName.lastIndexOf('.'),
            localName = globalName.substr(pos + 1);

        return Joints.namespace(globalName.substr(0, pos))[localName] = staticProperties;
    };
